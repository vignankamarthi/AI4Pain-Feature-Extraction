...
def process_single_dimension_tau(signal_data, dim, tau):
    ...
    try:
        ...
        if len(unique_values) < dim:
            logger.warning(f"Signal doesn't have enough unique values for dim={dim}, tau={tau}")
            return result
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # Log any warnings
            for warning in w:
                logger.warning(f"Warning in PE calculation (dim={dim}, tau={tau}): {warning.message}")
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # Log any warnings
            for warning in w:
                logger.warning(f"Warning in Fisher calculation (dim={dim}, tau={tau}): {warning.message}")
        ...
        if np.isnan(comp_value) or np.isinf(comp_value):
            logger.warning(f"Invalid complexity value: {comp_value} for dim={dim}, tau={tau}")
            comp_value = np.nan
            
        if np.isnan(pe_fisher_value) or np.isinf(pe_fisher_value):
            logger.warning(f"Invalid PE (Fisher) value: {pe_fisher_value} for dim={dim}, tau={tau}")
            pe_fisher_value = np.nan
            
        if np.isnan(fisher_value) or np.isinf(fisher_value):
            logger.warning(f"Invalid Fisher value: {fisher_value} for dim={dim}, tau={tau}")
            fisher_value = np.nan
        ...
        logger.error(f"Memory error processing signal with dim={dim}, tau={tau}: {str(e)}")
        return result
    ...
    for dim in sorted_dimensions:
        for tau in taus:
            try:
                ...
                logger.error(f"Unexpected error in process_signal for dim={dim}, tau={tau}: {str(e)}")
                logger.error(traceback.format_exc())
    ...
    try:
        ...
        try:
            if 'pe_fisher' in all_results.columns:
                ...
                valid_mask = np.isfinite(all_results['pe']) & np.isfinite(all_results['pe_fisher'])
                if valid_mask.sum() > 0:
                    pe_diff = (all_results.loc[valid_mask, 'pe'] - all_results.loc[valid_mask, 'pe_fisher']).abs()
                ...
                logger.info(f"NaN values in PE: {nan_pe_count}, NaN values in PE Fisher: {nan_pe_fisher_count}")
                print(f"NaN values in PE: {nan_pe_count}, NaN values in PE Fisher: {nan_pe_fisher_count}")
        except Exception as e:
