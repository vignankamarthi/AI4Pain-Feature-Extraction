...
def process_single_dimension_tau(signal_data, dim, tau):
    """
    Calculate metrics for a single dimension-tau combination.
    ...
    try:
        ...
        logger.error(f"Memory error processing signal with dim={dim}, tau={tau}")
...
def process_signal(signal_data, dimensions=[1, 2], taus=[1, 2, 3]):
    ...
    one at a time to manage memory.
